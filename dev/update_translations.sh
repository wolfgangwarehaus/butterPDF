...
#   dev/update_translations.sh es        # also CREATE butterpdf/i18n/butterpdf_es.ts
#
# pyside6-lupdate scans butterpdf/*.py for self.tr()/translate() strings and
...
# each .ts into the .qm the app ships (package data in butterpdf/i18n/).
#
...
mapfile -t SOURCES < <(find butterpdf -name '*.py' -not -path "$I18N_DIR/*" | sort)

# New-language bootstrap: seed an empty catalog so lupdate fills it below.
for lang in "$@"; do
    ts="$I18N_DIR/butterpdf_${lang}.ts"
    if [ ! -f "$ts" ]; then
...
shopt -s nullglob
TS_FILES=("$I18N_DIR"/butterpdf_*.ts)

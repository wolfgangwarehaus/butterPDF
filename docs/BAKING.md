# butterpdf — the baking phase

butterpdf has two halves. The first is the **creation medium** — the rise: a uniform
...
productizing what jellytoast proved in the field. butterpdf rises, then bakes, then
ships to the stores.
...
Everything below is downstream of that sentence. butterpdf already proved the pattern
...
1. **One metadata source.** A single schema-versioned table — `[tool.butterpdf.metadata]`
...
2. **Generate-then-verify, never hand-edit-then-test.** A `butterpdf bake` render step
   ...
   against a fresh render — any drift fails the build. (jellytoast lints; butterpdf
   regenerates.)
...
4. **The tag is the version.** Dynamic versioning (`setuptools-scm`): the single
   ...
   `butterpdf/_version.py`; CI resolves the version from the tag and injects it into
...
`[tool.butterpdf.metadata]` in `pyproject.toml` holds the union field set once,
...
| Projection | Value (butterpdf) | Consumed by |
|---|---|---|
| `windows_aumid` = `{org}.{app}` | `wolfgangwarehaus.butterpdf` | Win AUMID **and** winget `PackageIdentifier` seed **and** MSIX `Identity Name` |
| `app_id_base` = `io.github.{owner}.{app}` | `io.github.wolfgangwarehaus.butterpdf` | Flatpak manifest/metainfo/`.desktop`/icon names, deb+AppImage+AUR install paths, Wayland app_id, single-instance prefix |
| `cf_bundle_id` = `com.{org}.{app}` | `com.wolfgangwarehaus.butterpdf` | macOS `CFBundleIdentifier`, cask `zap` |
| `homepage_url` = `github.com/{owner}/{repo}` | …/wolfgangwarehaus/butterpdf | issues/releases/changelog/download URLs all derive |

The crux: `windows_aumid` is *already hardcoded* at `windows_shortcut.py:48`
(`APP_USER_MODEL_ID = "wolfgangwarehaus.butterpdf"`) and is exactly `{org}.{app}` — the
...
`runtime_deps` is the one genuinely app-specific block butterpdf can't own — but the
...
- **`configure(org, app, display_name)`** carries only *runtime* identity + its
  ...
  `design_tokens._load_font_scale` reads `QSettings("butterpdf","butterpdf")` at module
...
- **The overlap** is `org` / `app` / `display_name` / `version`: the seam exposes
  ...
  (butterpdf's analogue of `test_version_consistency.py`, extended to the id
...
- **MSIX / Store** — the first submission is **manual** via Partner Center (free
  ...
  the StartupTask autostart backend are generic enough to live in butterpdf's
...
- **macOS** *(a real goal — present but dormant)* — wired into the template now so
  butterpdf is mac-ready, gated off until you have an Apple account. Full chain:
...
PHASE 1  — on push, tags: v*
  release.yml: resolve version from the tag → butterpdf bake (render manifests)
...
`butterpdf/_version.py`; runtime `__version__` reads it. Cutting a release is
...
`butterpdf bake <name>` walks every `*.j2`, substitutes from `[tool.butterpdf.metadata]` +
...
> directly — it can't run a `.j2`), NOT rendered by `butterpdf bake`. They read the
...
```
pyproject.toml                      # [tool.butterpdf.metadata] — the one source
packaging/
  templates/
...
Three buckets, mapped onto the butterpdf roadmap:
...
2. **Build fresh as a framework capability (jellytoast hand-rolled per-channel; butterpdf
...
- **Inno `AppId` GUID** — `butterpdf bake` generates a fresh `uuid4` for a new fork; it is

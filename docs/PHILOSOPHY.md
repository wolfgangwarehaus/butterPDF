# butterpdf — philosophy

**butterpdf** is the shared base every wolfgang warehaus app is baked from. It's
...
layer on top. toast (jellytoast) was the first loaf; butterpdf is the starter
...
Three things make butterpdf good. Each is a principle, *why* it holds (research +
...
- **One decoupled bus.** `AppBus` (Qt signals): the UI emits intents, the
  backend reacts, neither holds a reference to the other. butterpdf ships only the
  generic chrome signals; an app extends the bus with its own. (jellytoast's
  `PlayerBus` grew to 60+ signals — butterpdf keeps the bus minimal on purpose.)
...
- **Core is PySide6-only.** Heavy or native dependencies (a PDF engine, mpv,
  ...
  out of the base. butterpdf core installs in seconds with zero native libs.
...
- **Flat, runnable layout.** A single `butterpdf/` package, `python -m butterpdf`, no
...
## What butterpdf refines vs. jellytoast

butterpdf is jellytoast's proven cross-platform/visual engine, minus the music,
...
- All network/disk I/O goes through `butterpdf.async_io` — never a raw
  `threading.Thread`.

...
butterpdf's kit ships accessible by default, and a test keeps forks honest. Three
...
`w.setProperty("a11y_exempt", True)` (bare butterpdf uses it zero times).
...
`butterpdf.keyboard_focus` recipe (`keyboard_focus_in/out`,

...
Every butterpdf app is born **drivable by an agent**. `BUTTERPDF_TEST_BRIDGE=1` opens
a dev-only, per-user local socket (`butterpdf/test_bridge.py`) whose JSON commands
...
jellytoast's autonomous QA legs on KDE Wayland, Windows, and macOS; butterpdf
...
TMPDIR=/tmp BUTTERPDF_TEST_BRIDGE=1 python -m butterpdf &   # wait for the window

# Windows (PowerShell) — named pipes, no TMPDIR concern:
$env:BUTTERPDF_TEST_BRIDGE=1; python -m butterpdf
```

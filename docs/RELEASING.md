...
from `[tool.butterpdf.metadata]` by `butterpdf bake`, never hand-edited (docs/BAKING.md).
...
- **PyPI Trusted Publishing** — add a pending publisher at
  ...
  (project `butterpdf`, owner `wolfgangwarehaus`, repo `butterpdf`, workflow
...
```sh
gh attestation verify <file> --repo wolfgangwarehaus/butterpdf

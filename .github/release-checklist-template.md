...
- [ ] **PyPI** — `pypi-publish.yml` uploaded `${V}`: https://pypi.org/project/butterpdf/${V}/
- [ ] Provenance verifies: `gh attestation verify <asset> --repo wolfgangwarehaus/butterpdf`

## Dormant / deferred — light up when configured (docs/TODO.md)
- [ ] **AUR** — `aur.yml` pushed `butterpdf ${V}` (needs `AUR_SSH_PRIVATE_KEY` + AUR registration)

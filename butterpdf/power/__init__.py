...
and :meth:`SleepInhibitor.release` when it's done. butterpdf owns the cross-platform
...
Backend (selected once per process, mirroring ``butterpdf.autostart`` /
``butterpdf.notifications``):
...
def _select_backend() -> ModuleType:
    ...
    from butterpdf.platform_compat import IS_LINUX, IS_WINDOWS

    if IS_WINDOWS:
        from butterpdf.power import _windows as backend
    elif IS_LINUX:
        from butterpdf.power import _linux as backend
    else:
        from butterpdf.power import _unsupported as backend
...
class SleepInhibitor(QObject):
    ...
    double as Qt signal slots), butterpdf provides the cross-platform HOW. Idempotent:

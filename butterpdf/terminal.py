...
class TerminalWidget(QWidget):
    ...
    def _materialize_image(img) -> "str | None":
        ...
        out = Path(tempfile.gettempdir()) / "butterpdf-terminal-images"
...
def claude_argv(resume: bool = False) -> list[str]:
    ...
    BUTTERPDF_AGENT_CMD (e.g. a wrapper, or a plain shell to try the terminal).

    ``resume`` adds ``--continue`` so a relaunched breadboard picks the last
    conversation in the project dir back up — the seam that lets butterpdf reload
    ...
    override = os.environ.get("BUTTERPDF_AGENT_CMD")
    if override:

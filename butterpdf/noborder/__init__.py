"""KWin ``noborder`` window rule for butterpdf's borderless main window.

On KDE Wayland butterpdf's main window is deliberately **server-side-decorated**
...
The rule is matched on the app's **wmclass** (``butterpdf.identity.app()``, the
...
exact title; butterpdf matches wmclass so the title stays free.)
...
feature — are intentionally not ported; butterpdf has no generic use for one yet.
...
if is_kde_wayland():
    from butterpdf.noborder import _kwin as _backend
else:
    from butterpdf.noborder import _unsupported as _backend

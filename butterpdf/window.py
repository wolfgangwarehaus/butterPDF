...
from butterpdf.platform_compat import IS_WINDOWS, is_kde_wayland
...
class _ResizeEdgeFilter(QObject):
    ...
    def eventFilter(self, obj, event):
        ...
        if edges == Qt.Edge(0):
            self._clear_cursor()
            return False
        # A press/hover on an interactive control near the edge is a click/drag,
        # not a resize — let it through. Buttons (titlebar window controls) AND
        # sliders (the scrollbar pill, which lives ON the right edge) both yield,
        # so the resize band doesn't steal the scrollbar's grab.

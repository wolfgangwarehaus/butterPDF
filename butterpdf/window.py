"""AppWindow — butterpdf's frosted, frameless, cross-platform main-window base.
...
from butterpdf.platform_compat import (
    IS_MACOS,
    IS_WINDOWS,
    is_kde_wayland,
    is_linux_wayland,
...
class AppWindow(QMainWindow):
    ...
    def __init__(self, *, title: str = "butterpdf"):
        super().__init__()
        ...
        # butterpdf/blur/_faux_frost.py.
        from butterpdf.blur._faux_frost import FauxFrost
        ...
        # (full-size content view, see butterpdf/macos_window.py) so the top bar
        # clears the floating traffic lights while the frosted backdrop shows
        # through behind them.
        if IS_MACOS:
            from butterpdf.macos_window import TITLEBAR_INSET
    ...
    def _make_top_bar(self, title: str):
        ...
        change). Default: the generic :class:`~butterpdf.top_bar.TopBar`."""
        from butterpdf.top_bar import TopBar
    ...
    def _apply_win_native_frame(self):
        if not self._win_frameless:
            return
        try:
            from butterpdf import win_frameless
    ...
    def _first_blur_pass(self):
        ...
        self._refresh_body_color()
        # BUTTERPDF_BLUR_DIAG=1 dumps the full surface truth once the window is
        ...
        if os.environ.get("BUTTERPDF_BLUR_DIAG") == "1":
            try:
        ...
        if IS_MACOS:
            from butterpdf.blur import _macos

...
OFF by default. Only stood up when ``BUTTERPDF_TEST_BRIDGE=1`` is set at launch
(wired in ``butterpdf.app.run_app``; the fork transform renames the env prefix).
...
BUTTERPDF_TEST_BRIDGE=1. It must never be enabled in a shipped/packaged build.
"""

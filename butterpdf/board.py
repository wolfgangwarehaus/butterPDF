"""``butterpdf board`` — the live maker surface (docs/TODO.md §THE DOUGH BOARD).

The board is the thing the maker INTERACTS WITH during each step of building
with butterpdf: one frosted window, one tab per phase (Ingredients → Baking →
...
**State is a file; the window is a view.** ``butterpdf-board.toml`` sits in the
...
# The board file is named after the APP (butterpdf-board.toml here, myapp-board.toml
# in a fork) — derived from the package, not a literal, so the fork rename can't
# half-apply it (module docstrings say "butterpdf-board.toml" but mean this).
...
def save(path: Path, board: dict) -> None:
    ...
    lines = [
        "# The butterpdf board — the live maker surface. The WINDOW (`butterpdf board`) and",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--init", action="store_true",
                        help="seed a fresh butterpdf-board.toml (refuses to overwrite)")

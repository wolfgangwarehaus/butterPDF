...
pip's entry-point launcher (``butterpdf.exe``) is a distlib stub whose
...
- Render ``%LOCALAPPDATA%/butterpdf/butterpdf.ico`` from the
...
- Write a per-user Start Menu shortcut (no elevation needed):
  ``%APPDATA%/Microsoft/Windows/Start Menu/Programs/butterpdf.lnk``
...
def _launcher_exe() -> Path | None:
    ...
    try:
        ...
        if (
            ...
            and cand.stem.lower().startswith("butterpdf")
            and cand.is_file()
        ):
            return cand
        scripts = Path(sys.executable).parent / "butterpdf.exe"
        if scripts.is_file():
            return scripts
    except Exception:
        pass
...
def _icon_path() -> Path:
    ...
    return base / "butterpdf" / "butterpdf.ico"


def _shortcut_path() -> Path:
    base = Path(os.environ.get("APPDATA") or (Path.home() / "AppData" / "Roaming"))
    return base / "Microsoft" / "Windows" / "Start Menu" / "Programs" / "butterpdf.lnk"
...
def _render_icon(path: Path) -> bool:
    ...
    try:
        ...
        from butterpdf.ui_helpers import make_app_icon
...
def _write_shortcut(lnk: Path, exe: Path, ico: Path) -> bool:
    ...
    try:
        ...
        if r.returncode != 0 or _STAMP_SENTINEL not in out:
            # INFO (not debug) so the `python -m butterpdf` console run

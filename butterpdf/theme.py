...
from butterpdf.platform_compat import IS_MACOS, IS_WINDOWS
...
class Theme:
    ...
    # separates Frosted from Solid. Applied via butterpdf/blur/; a silent
    ...
    # body_color_for() and butterpdf/blur/status().
...
def body_color_for(theme: "Theme", status, surface: str = "main") -> tuple:
    ...
    ``status`` (a ``butterpdf.blur.BlurStatus``).
    ...
    if not theme.blur or theme.fallback_body_alpha is None:
        return base
    # Import here so theme.py stays importable before butterpdf.blur is ready

...
Public API:
    ...
    notify(title, body="", icon=None, app_name=None, tag=None) -> None
        # app_name=None resolves to butterpdf.identity.app() (the live app slug)
...
`ToastNotificationManager`), bound to butterpdf's stamped AppUserModelID.
...
def _select_backend() -> ModuleType:
    ...
    the dispatch shape used by `butterpdf.autostart`."""
    global _backend
    if _backend is not None:
        return _backend

    from butterpdf.platform_compat import IS_LINUX, IS_MACOS, IS_WINDOWS

    if IS_LINUX:
        from butterpdf.notifications import _linux as backend
    elif IS_WINDOWS:
        try:
            from butterpdf.notifications import _windows as backend
        except Exception:
            from butterpdf.notifications import _unsupported as backend
    elif IS_MACOS:
        try:
            from butterpdf.notifications import _macos as backend
        except Exception:
            from butterpdf.notifications import _unsupported as backend
    else:
        from butterpdf.notifications import _unsupported as backend
...
def notify(
    ...
    (``butterpdf.identity.app()``) — pass it only to override. ``tag`` groups

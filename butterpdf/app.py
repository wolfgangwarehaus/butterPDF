...
def _diag_log_dir():
    ...
    from butterpdf import identity as _ident
...
def _enable_faulthandler() -> None:
    ...
    try:
        ...
        level = (_os.environ.get("BUTTERPDF_LOG_LEVEL") or "INFO").upper()
...
def run_app(content_factory, *, identity=None, single_instance=True) -> int:
    ...
    # macOS: set the application-menu name BEFORE QApplication builds the native
    # menu, so a from-source run reads the app name rather than "Python" (a frozen
    # .app already gets it from CFBundleName). No-op off macOS.
    if IS_MACOS:
        from butterpdf import macos_menubar
    ...
    # built on macOS, best-effort/no-op elsewhere.
    if IS_MACOS:
        from butterpdf import macos_menubar, macos_window
    ...
    # KWin `noborder` Force rule strips the visible decoration. Reconcile it BEFORE
    # show so a fresh launch never flashes a titlebar; idempotent + self-healing, a
    # no-op off KDE Wayland. `native_window_border` opts back into native chrome.
    from butterpdf import noborder
    ...
    # KWin drag-repaint effect — installed AFTER show (off the first-paint path; it

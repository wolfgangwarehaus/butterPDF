"""``butterpdf breadboard`` — the live maker surface (docs/TODO.md §THE BREADBOARD).

The breadboard is the thing the maker INTERACTS WITH during each step of
building with butterpdf: one frosted window, tabs across the top for the phases —
...
_PKG = (__package__ or "butterpdf").split(".")[0]

# The board file is named after the APP (butterpdf-breadboard.toml here,
...
def default_board(product: str) -> dict:
    ...
    return {
        ...
        "purpose": "",
        "ingredients": items(
            "Name + slug settled (butterpdf new done)",
...
def _make_view(path: Path, restore: dict | None = None):
    ...
    class BoardView(QWidget):
        ...
        def __init__(self) -> None:
            ...
            # the agent edits butterpdf), keeping your place + the agent session.
            self._reload_btn = ui_helpers.RoundedButton("⟳ Reload", variant="ghost")
            self._reload_btn.setToolTip(
                "Restart the breadboard on the latest butterpdf code without losing\n"
        ...
        def _on_theme(self) -> None:
            ...
            from butterpdf.selector import selector_qss
        ...
        def _prime_agent_button(self) -> None:
            from butterpdf import terminal
            ...
            if not terminal.is_supported():
                ...
                self._agent_btn.setToolTip(
                    ...
                    "(pip install 'butterpdf-base[terminal]'); not available here.")
            elif not terminal.agent_available():
                self._agent_btn.setEnabled(False)
                self._agent_btn.setToolTip(
                    "Claude Code (`claude`) isn't on PATH — install it, or set "
                    "BUTTERPDF_AGENT_CMD to the command to run.")
            else:
                self._agent_btn.setToolTip(
        ...
        def _request_reload(self) -> None:
            ...
            os.environ["BUTTERPDF_BREADBOARD_RESTORE"] = json.dumps(restore)
            ...
            try:
                ...
                os.environ.pop("BUTTERPDF_BREADBOARD_RESTORE", None)
                from PySide6.QtWidgets import QMessageBox
...
def main(argv: list[str] | None = None) -> int:
    ...
    from butterpdf.app import run_app
...
def _read_restore_env() -> dict | None:
    ...
    raw = os.environ.pop("BUTTERPDF_BREADBOARD_RESTORE", None)
    if not raw:
        return None
    try:
        data = json.loads(raw)

...
from butterpdf.design_tokens import rad
from butterpdf.icon_button import IconButton
from butterpdf.platform_compat import IS_MACOS

# ── Theme ────────────────────────────────────────────────────────────────────
# Palette + body fills come from the active Theme (butterpdf/theme.py).
# Constants are re-exported so existing `from butterpdf.ui_helpers import
...
from butterpdf.theme import get_active_theme, ink_alpha  # noqa: F401  (re-exported)
...
# (butterpdf/theme.py). The constants below are flat re-exports of those
...
def body_color_tuple(surface: str = "main") -> tuple:
    ...
    *verified* behind the window — glass (~67%) when it is, a near-opaque
    ...
    main / mini / dialog. Does NOT apply the main window's BUTTERPDF_OPAQUE override
    (that's main-window-only; the caller handles it). Never raises."""
    from butterpdf import blur
    from butterpdf.theme import body_color_for, get_active_theme
...
def frosted_fallback_active() -> bool:
    ...
    paint the faux-frost backdrop (``butterpdf/blur/_faux_frost.py``). Shared by the
    main window and mini player so they fall back identically. (The main window
    additionally suppresses it under the BUTTERPDF_OPAQUE override.)"""
    from butterpdf import blur
...
def _accent_rgb_tuple() -> tuple[int, int, int]:
    ...
    from butterpdf.theme import _hex_to_rgb
...
def apply_font_settings_live() -> None:
    ...
    from butterpdf import design_tokens as _dt
...
def _build_global_style() -> str:
    ...
    from butterpdf.design_tokens import RADIUS_LG, RADIUS_MD, RADIUS_SM, rad
    ...
    # NB hover tooltips are drawn by our custom popup (butterpdf/custom_tooltip),
...
QToolTip {{
    /* Hover tooltips are drawn by butterpdf/custom_tooltip (a top-level
...
def refresh_theme() -> str:
    ...
    ``from butterpdf.ui_helpers import ACCENT`` keeps a STALE reference,
    ...
    try:
        from butterpdf import color_tokens as _ct
...
# a surface might have imported `from butterpdf.ui_helpers import …` and
...
def _propagate_theme_constants() -> None:
    """Rebind the theme-token constants in every ``butterpdf.*`` module
    ...
    ``from butterpdf.ui_helpers import TEXT`` and bake it into a
    ...
    for mod_name, mod in list(sys.modules.items()):
        if mod is None or mod is src or not mod_name.startswith("butterpdf."):
            continue
...
def _propagate_font_constants() -> None:
    ...
    ``butterpdf.*`` module that imported them by value — the font-scale analog
    ...
    for mod_name, mod in list(sys.modules.items()):
        if mod is None or mod is _dt or not mod_name.startswith("butterpdf."):
            continue
        for name, value in values.items():
            if hasattr(mod, name):
                setattr(mod, name, value)
...
def apply_app_palette() -> None:
    ...
    text, which is invisible on a light butterpdf theme (the dark
    ...
    custom popup (butterpdf/custom_tooltip), which carries its own colour.
...
def skip_taskbar_x11(widget: QWidget):
    ...
    from butterpdf.platform_compat import is_x11
    ...
    from butterpdf.async_io import run_async
...
def _load_app_icon_svg_bytes() -> bytes:
    ...
    try:
        ...
        res = _ir.files("butterpdf.assets").joinpath("butterpdf.svg")
        if not res.is_file():
            return b""
...
def _draw_placeholder_icon(size: int) -> QPixmap:
    ...
    try:
        from butterpdf.theme import get_active_theme as _gt
...
def make_app_icon(size: int = 64) -> QPixmap:
    """butterpdf logo, rasterized from the bundled
    ``butterpdf/assets/butterpdf.svg`` at the requested pixel size.
...
class EmptyState(QWidget):
    ...
    def _apply_styling(self) -> None:
        ...
        from butterpdf.design_tokens import TYPE_BODY, TYPE_CAPTION, type_qss
...
def apply_elevated_blur(widget, corner_radius: int = 0) -> bool:
    ...
    """
    try:
        from butterpdf import blur as _blur
        from butterpdf.theme import get_active_theme
...
class CircleSwatch(QPushButton):
    ...
    butterpdf and butterPDF). This paints with QPainter instead: the fill is one
...
class RoundedButton(QPushButton):
    ...
    def __init__(self, text: str = "", *, variant: str = "ghost", radius: int = 8,
        ...
        try:
            from butterpdf.bus import register_for_theme
...
def frost_scroll_surface(area) -> None:
    ...
    butterpdf hallmark. Generic over any ``QAbstractScrollArea`` (QScrollArea, QListView,
    a Qt PDF view, …)."""
    area.setStyleSheet(

...
``butterpdf.__version__`` — and the user hasn't dismissed that exact version — it
...
stamp (the ``BUTTERPDF_CHANNEL`` env var, default ``"source"`` → checks ON, the safe
direction — a store package sets ``BUTTERPDF_CHANNEL=msix``/``aur``/… to disable).
...
from butterpdf.async_io import get_qnam
from butterpdf.platform_compat import is_macos_sandboxed, is_msix_packaged
...
def get_channel() -> str:
    ...
    to ``BUTTERPDF_CHANNEL`` (default ``"source"``). Never raises."""
    try:
        if is_msix_packaged():
            return "msix"
        ...
        pass
    return (os.environ.get("BUTTERPDF_CHANNEL") or "").strip().lower() or "source"

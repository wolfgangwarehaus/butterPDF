...
``dev/update_translations.sh`` extracts them into ``butterpdf/i18n/*.ts``
...
Two translators install per non-English language:
  ...
  * The app catalog (``butterpdf_<lang>.qm``) shipped as package data in
    ``butterpdf/i18n/``. The literal ``butterpdf`` in the catalog name is deliberate:
    ``butterpdf new``'s whole-word identity replace renames it with the package,
...
# Keep in sync with butterpdf/i18n/butterpdf_<code>.ts — the Settings dropdown builds
# from this list. butterpdf ships none; a fork adds a row per catalog, e.g.:
#     ("es", "Spanish", "Español"),
...
def install(app) -> str:
    ...
    try:
        ...
        qm = res.files("butterpdf.i18n").joinpath(f"butterpdf_{lang}.qm")

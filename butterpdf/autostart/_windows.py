...
checkout falls back to ``pythonw.exe -m butterpdf`` (then plain
...
_VALUE_NAME = "butterpdf"


def _launch_command() -> str:
    """The command line to register: the launcher exe if we have one,
    else a ``-m butterpdf`` invocation, preferring pythonw so a login
    launch doesn't flash a console."""
    from butterpdf.windows_shortcut import _launcher_exe

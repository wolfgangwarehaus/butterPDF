"""XDG autostart backend. Manages ~/.config/autostart/butterpdf.desktop —
...
Strategy:
- Enable: copy ~/.local/share/applications/butterpdf.desktop into the
...
_AUTOSTART_FILE = _AUTOSTART_DIR / "butterpdf.desktop"
_SOURCE_DESKTOP = Path.home() / ".local" / "share" / "applications" / "butterpdf.desktop"
...
def _synth_desktop_entry() -> str:
    ...
    the installed butterpdf package (`python -m butterpdf`). Used when
    the canonical entry under ~/.local/share/applications is missing."""
    # parent.parent = the butterpdf package dir; its parent is whatever
    ...
    return (
        ...
        f'Exec={interpreter} -m butterpdf\n'

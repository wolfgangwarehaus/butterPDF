"""butterpdf's build-time metadata — the one source the oven renders every manifest from.

Build-time counterpart to :mod:`butterpdf.identity` (runtime). ``identity`` owns the
LIVE app identity (org / app / display_name) and the reverse-DNS id projections
the RUNNING QApplication needs; this module reads the ``[tool.butterpdf.metadata]``
sidecar in ``pyproject.toml`` — the descriptive / store fields a future
``butterpdf bake`` stamps into the ``.desktop`` entry, the AppStream metainfo, deb
...
class MetadataError(RuntimeError):
    """The ``[tool.butterpdf.metadata]`` sidecar is missing or malformed."""
...
def _find_pyproject() -> Path:
    ...
    for parent in Path(__file__).resolve().parents:
        ...
        if candidate.is_file():
            return candidate
    raise MetadataError(
        "pyproject.toml not found above butterpdf/metadata.py — the baking-phase "
...
@lru_cache(maxsize=1)
def load() -> dict:
    """The raw ``[tool.butterpdf.metadata]`` table, with the resolved package
    ...
    try:
        meta = dict(data["tool"]["butterpdf"]["metadata"])
    except KeyError as exc:
        raise MetadataError(f"[tool.butterpdf.metadata] missing from {pyproject}") from exc
    ...
    # the Beat-2 `butterpdf bake` renderer — until then metadata_version is a forward
    ...
    if "metadata_version" not in meta:
        raise MetadataError(f"[tool.butterpdf.metadata] has no metadata_version in {pyproject}")
    for key in ("org_slug", "app_slug", "github_owner", "repo_name"):
        if key not in meta:
            raise MetadataError(f"[tool.butterpdf.metadata] missing required key {key!r} in {pyproject}")

    from butterpdf import __version__

    meta.setdefault("version", __version__)
    # The PyPI DISTRIBUTION name ([project].name) — distinct from app_slug when
    # the slug is taken on PyPI (butterpdf publishes as `butterpdf-base`). Folded in from

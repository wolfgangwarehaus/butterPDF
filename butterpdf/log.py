...
Level comes from the ``BUTTERPDF_LOG`` env var (``debug`` / ``info`` / ``warning``,
default ``info``) — nothing is persisted in Settings; a support request is
"relaunch with BUTTERPDF_LOG=debug", not a toggle hunt. ``run_app`` calls
...
def _level_from_env() -> int:
    """Root level from ``BUTTERPDF_LOG`` (debug/info/warning/error), default INFO."""
    name = (os.environ.get("BUTTERPDF_LOG") or "info").strip().upper()
...
def log_file_path() -> Path | None:
    ...
    (``butterpdf.diagnostics`` tails this for the support report.)"""
    return _file_path

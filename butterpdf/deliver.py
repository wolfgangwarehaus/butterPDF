"""``butterpdf deliver`` — the Delivery-phase walkthroughs (docs/DELIVERY.md).
...
performed.
    ...
    python -m butterpdf.deliver              # the board: every channel, its state
    python -m butterpdf.deliver pypi         # walk ONE channel's next steps
    python -m butterpdf.deliver --release    # the update lap (the Improvements loop)
...
class Ctx:
    ...
    # the bare name is squatted (butterpdf publishes as butterpdf-base; the import and
    ...
    def pypi_state(self) -> str | None:
        ...
        reads as LIVE (found live: PyPI 'butterpdf' is a 2015-era third-party
        package)."""

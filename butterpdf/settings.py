...
butterpdf keeps a tiny, generic settings surface: theme, accent, font scale,
...
``butterpdf.identity`` (or via ``butterpdf.configure(...)``); this reads from there.
"""
...
class Settings:
    ...
    def auto_hide_scrollbars(self) -> bool:
        ...
        SETTING is butterpdf-base; an app wires install_autofade_scrollbars on its own
        scroll areas when this is on (butterpdf can't know which scroll areas an app has)."""
    ...
    def check_for_updates(self) -> bool:
        # Daily GitHub latest-release check (butterpdf.updates) — the chip in
        # the top bar. On by default; auto-updating channels (Store / MAS /
        # AUR) are suppressed regardless of this toggle.

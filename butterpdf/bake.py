"""``butterpdf bake`` — render the packaging tree from the one metadata source.
...
(filenames carry ``{{ app_id_base }}`` etc.) from :func:`butterpdf.metadata.context`,
...
orphan (a committed file no template produces). A CI gate (``butterpdf bake --check``,
mirrored by ``tests/test_bake.py``) fails the build on any drift. jellytoast
*lints* its manifests after the fact; butterpdf regenerates and proves nothing drifted.
...
def _environment(templates_dir: Path):
    try:
        ...
        raise BakeError(
            "butterpdf bake needs Jinja2 — install the bake extra: pip install 'butterpdf[bake]'"
        ) from exc
    return Environment(
...
def check(packaging_dir: Path, ctx: dict | None = None) -> list[str]:
    ...
    for rel, body in rendered.items():
        dest = packaging_dir / rel
        if not dest.is_file():
            drift.append(f"missing (run `butterpdf bake`): {rel}")
            continue
        if dest.read_bytes() != body.encode("utf-8"):
            drift.append(f"drift (re-run `butterpdf bake`): {rel}")
        elif dest.suffix == ".sh" and not (dest.stat().st_mode & stat.S_IXUSR):
            drift.append(f"not executable (re-run `butterpdf bake`): {rel}")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="butterpdf-bake",
        description="Render the packaging tree from [tool.butterpdf.metadata] (docs/BAKING.md §2).",
    )
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    if args.release_version and args.packaging is None:
        ...
        # break `butterpdf bake --check`. Force an explicit target (CI renders into
    ...
    try:
        if args.check:
            ...
            if drift:
                print("packaging out of sync with [tool.butterpdf.metadata]:", file=sys.stderr)
                print("  " + "\n  ".join(drift), file=sys.stderr)
                return 1
            print("packaging is in sync with [tool.butterpdf.metadata].")
            return 0
        rendered = write(packaging_dir, ctx)
    except BakeError as exc:
        print(f"butterpdf bake: {exc}", file=sys.stderr)
        return 1

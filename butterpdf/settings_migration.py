...
/ theme-axes one-shots that healed real installs); butterpdf ships the generic
...
The add-a-migration recipe (a fork's, or butterpdf's own):
...
def _open_qsettings() -> QSettings:
    ...
    pair ``butterpdf.settings.Settings`` opens. The (format, scope, org, app) ctor

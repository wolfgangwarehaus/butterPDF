"""Boot-phase wall-clock instrumentation, opt-in via ``BUTTERPDF_BOOT_TIMING=1``.
...
Usage on any install:
    BUTTERPDF_BOOT_TIMING=1 butterpdf            (fish/bash)
    $env:BUTTERPDF_BOOT_TIMING="1"; butterpdf    (PowerShell)
"""
...
logger = logging.getLogger("butterpdf.boot")

_ENABLED = os.environ.get("BUTTERPDF_BOOT_TIMING") == "1"
_T0 = time.perf_counter()
_LAST = _T0


def mark(label: str) -> None:
    """Record a boot milestone. No-op unless BUTTERPDF_BOOT_TIMING=1."""
    global _LAST
    if not _ENABLED:
        return

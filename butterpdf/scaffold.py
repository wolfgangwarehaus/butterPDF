"""``butterpdf new`` — scaffold a fresh OWNED app from this butterpdf checkout.

``butterpdf-new <slug> --display "My App" --org <owner> [--summary "…"]`` turns THIS
clone of butterpdf into an owned ``<slug>`` app: it renames the package, rewrites every
butterpdf identity reference, mints a fresh installer id, strips butterpdf's own dev
...
butterpdf*: the one command from a butterpdf checkout to a real app repo.

Identity is centralized (the seam + the ``[tool.butterpdf.metadata]`` sidecar), so the
...
from butterpdf import metadata

# butterpdf's own dev scaffolding — a fork is an app, not the base, so these go.
# Includes the tests for base-only tooling that `dev/` carries (sync_loaf, the
# `butterpdf new` verb itself): once dev/ and the loaf AGENTS template are stripped,
# those tests reference files that no longer exist, so they must go too — else
# `butterpdf new`'s own validate step fails on the fresh loaf. docs/BREADBOARD.md is
# butterpdf's internal moat strategy, not loaf-facing. The base's own
...
def _replace_in_tree(root: Path, pairs: list[tuple[str, str]]) -> None:
    ...
    # …and the lowercase_snake prefix (``butterpdf_<code>.qm`` i18n catalog
...
def _identity_org_repairs(
    ...
    case: butterpdf's own org and owner coincide ("wolfgangwarehaus"), so the
...
def _scaffold(root: Path, slug: str, display: str, new_org: str, new_owner: str, summary: str | None) -> None:
    ...
    # 1. strip butterpdf's own dev scaffolding + the base's own breadboard file
    ...
    # The PyPI distribution name: butterpdf publishes as "butterpdf-base" (the bare name
    ...
    #    (io.github.…butterpdf.desktop), which the content-replace can't touch, so the
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="butterpdf-new",
        description="Scaffold a fresh OWNED app from this butterpdf checkout (destructive, in-place).",
    )
    ...
    parser.add_argument("--org", help="org / vendor slug (default: keep butterpdf's)")
    ...
    if not (root / sidecar["app_slug"]).is_dir():
        parser.error(f"no package dir {sidecar['app_slug']}/ at {root} — run inside a butterpdf checkout")

    display = args.display or slug
    new_org = args.org or sidecar["org_slug"]
    new_owner = args.owner or new_org

    print(f"butterpdf new → scaffolding '{slug}' (display '{display}', org '{new_org}') in {root}")

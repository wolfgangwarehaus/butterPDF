"""``butterpdf rig`` — rig up the app and probe what the platform actually saw.
...
read back ground truth instead of eyeballing.
    ...
    python -m butterpdf.rig boot            # offscreen boot smoke (any machine, CI)
    python -m butterpdf.rig probe           # live KDE probe: app_id / noborder / X11 class
    python -m butterpdf.rig shot [-o f.png] # screenshot the live window (visual review)
    python -m butterpdf.rig baseline        # visual-bump gate: grabs vs the goldens
    python -m butterpdf.rig baseline --update   # (re)write the goldens

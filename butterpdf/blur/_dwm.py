...
(``ACCENT_ENABLE_ACRYLICBLURBEHIND``); see ``apply_acrylic``. ``BUTTERPDF_NO_WIN_BLUR``
...
main window pairs the Acrylic path with a NON-layered window (butterpdf/app.py's
...
from butterpdf.platform_compat import IS_WINDOWS
...
def _acrylic_tint(dark: bool, elevated: bool = False) -> int:
    """Acrylic tint (AABBGGRR). BUTTERPDF_WIN_BLUR_ALPHA overrides just the alpha
    ...
    as the single tint source on every platform. BUTTERPDF_WIN_POPUP_BLUR_ALPHA
    tunes it live for eyeball calibration."""
    base = _ACRYLIC_TINT_DARK if dark else _ACRYLIC_TINT_LIGHT
    if elevated:
        try:
            a = int(os.environ.get("BUTTERPDF_WIN_POPUP_BLUR_ALPHA", "1"))
        except ValueError:
            a = 1
        return (max(1, min(255, a)) << 24) | (base & 0x00FFFFFF)
    try:
        a = int(os.environ.get("BUTTERPDF_WIN_BLUR_ALPHA", ""))
    except ValueError:
        return base
...
def apply(
    ...
    when ``BUTTERPDF_NO_WIN_BLUR`` is set. ``corner_radius > 0`` additionally asks DWM to round
    ...
    try:
        ...
        # Real frosted-glass blur — the DEFAULT on Windows (BUTTERPDF_NO_WIN_BLUR
        ...
        # a NON-layered window (butterpdf/app.py `_win_blur` drops
        ...
        if not os.environ.get("BUTTERPDF_NO_WIN_BLUR"):
...
def probe():
    ...
    see-through). See butterpdf/blur/__init__.py."""
    from butterpdf.blur import BlurStatus
...
def reason(status) -> str:
    ...
    if status == BlurStatus.ACTIVE:
        # Default is the real Acrylic accent blur; BUTTERPDF_NO_WIN_BLUR falls back
        # to the (flat) Mica system-backdrop.
        return (
            "Windows 11 Mica backdrop active"
            if os.environ.get("BUTTERPDF_NO_WIN_BLUR")

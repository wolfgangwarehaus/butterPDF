...
See butterpdf/theme.body_color_for() and docs/research/portable_blur.md.
...
Wayfire, SwayFX), the user can target butterpdf in their own window rules:
our Wayland app_id is the stable string "butterpdf" (set via
setDesktopFileName).
"""
...
if IS_LINUX:
    from butterpdf.blur import _kwin as _backend
elif IS_WINDOWS:  # pragma: no cover - exercised on Windows
    from butterpdf.blur import _dwm as _backend
elif IS_MACOS:  # pragma: no cover - exercised on macOS
    from butterpdf.blur import _macos as _backend
else:  # pragma: no cover
    from butterpdf.blur import _unsupported as _backend
...
def apply(
    ...
    try:
        ...
        # backend (butterpdf/blur/_macos.py): it installs a "behind window" effect

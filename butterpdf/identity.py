"""butterpdf's identity seam — the one place app identity lives.
...
call :func:`configure` before importing anything heavy::
    ...
    import butterpdf
    butterpdf.configure(org="acme", app="myapp", display_name="My App")
    from butterpdf.app import main
    main()

Why it must come first, and why a function rather than ``QApplication``: the
font-scale loader in :mod:`butterpdf.design_tokens` reads ``QSettings(org, app)`` at
...
# butterpdf's own identity. `org` is the organization / vendor (Qt's
...
def configure(
    *,
    ...
    """Set the app identity. Call ONCE, before importing ``butterpdf.design_tokens``
    or ``butterpdf.app`` (the font-scale loader reads the org/app pair at import
...
# build-time renderer (butterpdf.metadata) calls the SAME pure helpers with the

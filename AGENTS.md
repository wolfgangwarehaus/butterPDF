...
You are an AI agent in the **butterPDF** checkout — an app **baked from
...
2. Read **`butterpdf-breadboard.toml`** — the live maker board (goals + the
   ...
   wind-down. `butterpdf-breadboard` opens the window.
...
aesthetic, delivery targets — the `[tool.butterpdf.metadata]` sidecar in
...
`butterpdf-breadboard.toml` is the contract between three parties — the maker,
the breadboard **window**, and you. All three read AND write it, and it's
git-tracked so its history is the project's. The window (`butterpdf-breadboard`)
...
- **Baking items also carry `priority`** = `now` | `next` | `later` (the kanban
  columns); new baking work defaults to `next`. `butterpdf-breadboard --init`
  seeds a fresh board.

## Conventions inherited from the base (hard-earned — keep them)

- **Identity flows from ONE seam**: `butterpdf/identity.py` (runtime) and the
  `[tool.butterpdf.metadata]` sidecar (build-time). Never write a literal
...
- **KDE chrome**: KDE Wayland stays **decorated + a noborder KWin rule**
  (`butterpdf/noborder/`), NOT `FramelessWindowHint` — the frameless shortcut
...
- **The validation gate** before any commit:
  `ruff check .` + `pytest` + `python -m butterpdf.bake --check`.
...
| `docs/BAKING.md` | the `butterpdf bake` renderer + the channel matrix |

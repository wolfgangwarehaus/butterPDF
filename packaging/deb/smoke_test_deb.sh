...
# by `butterpdf bake` — edit the template, never this file.
...
DEBIAN_FRONTEND=noninteractive apt-get install -y -qq \
  /src/dist/butterpdf_*_amd64.deb \
...
test -x /usr/bin/butterpdf && test -x "$(readlink -f /usr/bin/butterpdf)"
echo "OK: /usr/bin/butterpdf resolves to the bundle"

# Metadata validators (catch build_deb.sh staging drift). appstream is advisory.
desktop-file-validate /usr/share/applications/io.github.wolfgangwarehaus.butterpdf.desktop
echo "OK: .desktop validates"
appstreamcli validate --no-net /usr/share/metainfo/io.github.wolfgangwarehaus.butterpdf.metainfo.xml || true
...
set +e
QT_DEBUG_PLUGINS=1 QT_QPA_PLATFORM=xcb xvfb-run -a timeout 15 /usr/bin/butterpdf >/tmp/boot-xcb.log 2>&1
rc=$?
set -e
if [ "$rc" != 124 ] && [ "$rc" != 0 ]; then

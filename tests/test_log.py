"""butterpdf.log — the opt-in rotating file log.
...
def test_install_creates_rotating_file_log(tmp_path):
    ...
    logging.getLogger("butterpdf.test").info("hello from the suite")
...
def test_dough_log_env_sets_debug(monkeypatch):
    monkeypatch.setenv("BUTTERPDF_LOG", "debug")
    dlog.install()
...
def test_rotation_caps_file_size(tmp_path, monkeypatch):
    ...
    dlog.install()
    lg = logging.getLogger("butterpdf.rotate")
    for i in range(200):

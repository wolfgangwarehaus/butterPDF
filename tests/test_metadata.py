...
(``butterpdf bake``) exists.

Four guards:

1. **Sidecar shape** — ``[tool.butterpdf.metadata]`` loads, is schema-versioned, and
...
4. **No re-literalised ids** — no composite reverse-DNS / AUMID id appears as a
   ...
   formula. notifications/_windows.py used to carry ``"wolfgangwarehaus.butterpdf"``;
...
import butterpdf
from butterpdf import identity, metadata
...
def test_sidecar_slugs_match_the_runtime_seam(sidecar: dict) -> None:
    ...
    the same strings butterpdf.identity exposes at runtime."""
...
def test_projection_values_are_canonical() -> None:
    proj = metadata.projections()
    assert proj["app_id_base"] == "io.github.wolfgangwarehaus.butterpdf"
    assert proj["windows_aumid"] == "wolfgangwarehaus.butterpdf"
    assert proj["cf_bundle_id"] == "com.wolfgangwarehaus.butterpdf"
    ...
    assert proj["desktop_id"] == "io.github.wolfgangwarehaus.butterpdf"
    assert proj["homepage_url"] == "https://github.com/wolfgangwarehaus/butterpdf"
    assert proj["client_identity"] == f"butterpdf/{butterpdf.__version__} (+{proj['homepage_url']})"
...
def test_project_entry_point_matches_sidecar(pyproject: dict, sidecar: dict) -> None:
    ...
    assert pyproject["project"]["gui-scripts"]["butterpdf"] == sidecar["entry_point"]
...
def test_no_hardcoded_composite_ids_outside_the_seam() -> None:
    """No source outside butterpdf/identity.py (where the formula lives) may carry a
    ...
    org, app = "wolfgangwarehaus", "butterpdf"  # butterpdf's frozen canonical org/app
    patterns = [
    ...
    pkg_dir = Path(butterpdf.__file__).parent
    exempt = {"identity.py", "_version.py"}  # the formula home; the generated file
    # NOTE: scope is the butterpdf/ *.py package only. When the Beat-2 `butterpdf bake`
    ...
    # winget YAML / PKGBUILD) — those aren't .py and don't live under butterpdf/.
    offenders: list[str] = []
    ...
    assert not offenders, "re-literalised composite ids (use butterpdf.identity):\n  " + "\n  ".join(
        offenders
    )

...
1. **In sync** — a fresh render equals what's committed (the ``butterpdf bake
...
import butterpdf
from butterpdf import bake, identity, metadata
...
def test_packaging_is_in_sync(packaging_dir: Path) -> None:
    ...
    assert not drift, "packaging out of sync (run `butterpdf bake`):\n  " + "\n  ".join(drift)
...
def test_release_render_defaults_a_valid_date(tmp_path: Path, packaging_dir: Path) -> None:
    """`butterpdf bake --release-version` WITHOUT --release-date self-defaults a real
...
def test_check_ignores_release_version(packaging_dir: Path) -> None:
    """`butterpdf bake --check --release-version X` must NOT inject a <release> or
...
def test_bake_cli_injects_release(tmp_path: Path, packaging_dir: Path) -> None:
    """`butterpdf bake --release-version` writes the <release> block (the release-time
...
def test_templates_use_projections_not_literal_ids(packaging_dir: Path) -> None:
    ...
    hardcoded composite id — else a fork renaming itself silently ships butterpdf's
    ids into its manifests (the rendered output legitimately carries the literals;
    only the .j2 sources are scanned)."""
    import re

    org, app = "wolfgangwarehaus", "butterpdf"  # frozen canonical org/app
    patterns = [

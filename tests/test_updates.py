"""butterpdf.updates + butterpdf.update_chip — the daily release check and its chip.
...
def isolated_settings(tmp_path, monkeypatch):
    ...
    process singleton for both butterpdf.settings and the updates module."""
    import butterpdf.settings as smod
...
def test_channel_env_stamp(monkeypatch):
    ...
    monkeypatch.delenv("BUTTERPDF_CHANNEL", raising=False)
    assert updates.get_channel() == "source"
    monkeypatch.setenv("BUTTERPDF_CHANNEL", "AUR")
...
def test_runtime_probe_wins_over_env(monkeypatch):
    ...
    monkeypatch.setenv("BUTTERPDF_CHANNEL", "deb")
...
def test_releases_api_uses_sidecar_repo():
    # From this checkout the metadata sidecar names wolfgangwarehaus/butterpdf.
    assert updates._releases_api() == (
        "https://api.github.com/repos/wolfgangwarehaus/butterpdf/releases/latest"
    )


def test_repo_falls_back_to_identity(monkeypatch):
    import butterpdf.metadata as md

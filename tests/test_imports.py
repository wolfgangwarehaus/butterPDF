...
1. **Import-smoke** — every public ``butterpdf`` module imports cleanly on *this*
...
2. **Phantom-import guard** — no statement anywhere in the package names a
   ``butterpdf.<x>`` module that doesn't ship. This is a *static* AST walk, so it
...
def _public_modules() -> list[str]:
    mods: list[str] = []
    for info in pkgutil.walk_packages(butterpdf.__path__, prefix="butterpdf."):
...
def test_public_module_imports(modname: str) -> None:
    """Every public butterpdf module imports without error on this platform."""
...
def _dough_import_targets() -> list[tuple[str, Path, int]]:
    """Every absolute ``butterpdf.<x>`` an import statement names, anywhere in the
    ...
    for py in sorted(pkg_dir.rglob("*.py")):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                mod = node.module
                if node.level == 0 and mod and (mod == "butterpdf" or mod.startswith("butterpdf.")):
                    targets.append((mod, py, node.lineno))
            elif isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name == "butterpdf" or alias.name.startswith("butterpdf."):
                        targets.append((alias.name, py, node.lineno))
    return targets


# Modules that ship in a BUILT wheel but are legitimately absent from a raw
# source checkout — setuptools-scm writes butterpdf/_version.py at build time (the
...
def _is_phantom(target: str, *, origin: Path) -> bool:
    ...
    if target in _GENERATED and origin == Path(butterpdf.__file__):
        return False
    try:
...
def test_no_phantom_dough_imports() -> None:
    """No statement imports a ``butterpdf.<x>`` module that doesn't ship (the
    ...
    assert not phantoms, "phantom butterpdf.* imports:\n  " + "\n  ".join(phantoms)

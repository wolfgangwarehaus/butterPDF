...
Covers the boot-time layer only: which language ``butterpdf.i18n`` resolves
(Settings override → system locale → English fallback) and that install()
degrades cleanly while no catalog ships (bare butterpdf's SHIPPED_LANGUAGES is
...
class TestInstall:
    def test_no_shipped_catalog_stays_english(self, qapp, _lang_settings):
        # Bare butterpdf ships no catalogs — an explicit pick degrades to English

...
The app runs as an actual SUBPROCESS (offscreen, BUTTERPDF_TEST_BRIDGE=1) and the
...
butterpdf.configure(org="butterpdf-tests", app=sys.argv[1], display_name="bridge probe")

from butterpdf import test_bridge
...
def bridge(qapp, tmp_path_factory):
    ...
    slug = f"butterpdf-bridgetest-{uuid.uuid4().hex[:8]}"
...
def test_socket_name_follows_identity(qapp):
    from butterpdf import identity, test_bridge
...
def test_coerce_degrades_to_repr():
    from butterpdf.test_bridge import _coerce

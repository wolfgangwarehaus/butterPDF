"""The Delivery walkthroughs (butterpdf/deliver.py) — state is DETECTED, offline-
...
from butterpdf.deliver import Ctx
...
def _pypi_payload(home: str) -> dict:
    return {"info": {"name": "butterpdf", "home_page": home, "project_urls": None}}
...
def test_pypi_name_conflict_is_flagged_not_live(monkeypatch):
    ...
    pypi 'butterpdf' is a 2015-era third-party package)."""
    monkeypatch.setattr(deliver, "_run", lambda cmd: None)
    monkeypatch.setattr(deliver, "_gh_json", lambda args: None)
    monkeypatch.setattr(
        deliver, "_http_json",
...
def test_pypi_probes_use_the_distribution_name(monkeypatch):
    """butterpdf publishes as butterpdf-base (the bare name is squatted): every PyPI
    ...
    assert any("pypi.org/pypi/butterpdf-base/json" in u for u in urls)
    out = deliver.walkthrough(c, "pypi")
    assert "PyPI project name:  butterpdf-base" in out

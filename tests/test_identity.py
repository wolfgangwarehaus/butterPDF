...
These lock the runtime half of the seam: butterpdf's own defaults, the deterministic
...
def test_default_identity() -> None:
    ...
    assert identity.app() == "butterpdf"
    assert identity.display_name() == "butterPDF"


def test_windows_aumid_projection() -> None:
    # {org}.{app} — also the winget PackageIdentifier seed + MSIX Identity Name.
    assert identity.windows_aumid() == "wolfgangwarehaus.butterpdf"
...
def test_reverse_dns_projections() -> None:
    ...
    assert identity.app_id_base() == "io.github.wolfgangwarehaus.butterpdf"
    assert identity.cf_bundle_id() == "com.wolfgangwarehaus.butterpdf"
    assert identity.desktop_id() == identity.app_id_base()


def test_pure_projection_helpers() -> None:
    """The pure helpers the build-time renderer (butterpdf.metadata) shares with the
...
def test_owner_overrides_independently_of_org() -> None:
    ...
    account than its vendor slug). Restores butterpdf's identity after — including
...
def test_settings_handle_keys_off_identity() -> None:
    ...
    from butterpdf.settings import Settings
...
def test_configure_reidentifies_then_restores() -> None:
    ...
    butterpdf's own identity so the rest of the suite is unaffected."""

"""Unit coverage for `butterpdf new` (butterpdf.scaffold) — the pure pieces.
...
from butterpdf import scaffold


@pytest.mark.parametrize("bad", ["Butter", "butter-pdf", "1app", "butter pdf", "", "butterpdf.x"])
...
def test_replace_in_tree_is_whole_word(tmp_path: Path) -> None:
    ...
    f.write_text(
        ...
        "x = _dough_helper()  # the butterpdf brand\n"
        "g = 'butterpdf._version'\n",
        encoding="utf-8",
    )
    scaffold._replace_in_tree(tmp_path, [("butterpdf", "butterpdf")])
...
def test_replace_in_tree_skips_binary_and_caches(tmp_path: Path) -> None:
    ...
    cached.write_bytes(b"butterpdf\x00binary")
    scaffold._replace_in_tree(tmp_path, [("butterpdf", "butterpdf")])
    assert cached.read_bytes() == b"butterpdf\x00binary"  # untouched

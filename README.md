...
butterpdf is a **fork-and-own starter**, not a dependency. Clone it, rename it,
...
- **Languages for free** — Qt translation catalogs wired at boot, plural-aware
  strings, locale-safe number/date/duration formatting (`butterpdf.i18n.fmt`),
...
- **Structured logging** — an opt-in rotating file log (~1 MB × 3) in the app's
  state dir, console noise capped at WARNING, `BUTTERPDF_LOG=debug` to turn it up.
...
- **Second-launch file forwarding** — double-click a PDF while butterPDF is
  open and the running window raises + opens it (`AppBus.files_received`).
...
```bash
git clone https://github.com/wolfgangwarehaus/butterpdf.git
cd butterpdf
pip install -e .
python -m butterpdf          # or: butterpdf  (the console script)
```

Runs straight from a checkout — the flat layout means no install is strictly
required (`python -m butterpdf`).

## Make it yours

1. **Rename the package + set your identity.** Rename the `butterpdf/` package to your
   app, then set your identity in **one** place — `butterpdf/identity.py` (`org`, `app`,
   ...
   control, call `butterpdf.configure(org=…, app=…, display_name=…)` once, before
   importing the app — the font-scale loader reads identity at import time.)
2. **Boot your content.** Call `butterpdf.run_app(lambda window: YourWidget(window))`
   ...
   (Or, for the quickest start, just edit `butterpdf/app.py::_placeholder`.)
3. **Extend the bus.** Add your app's signals by subclassing `AppBus`
   (`butterpdf/bus.py`); the base stays minimal.
